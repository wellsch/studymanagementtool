...
def generate_study_plan(course, notes):
    prompt = build_prompt(course, notes)
    response = get_response(prompt)
    return json.loads(response)

# Build a prompt based on the notes files
def build_prompt(course, notes):
    labeled_notes = ""
...
You must return each the study plans sequentially with respect to the order they appeared in the notes. Each note represents one class day and should correspond to 2-5 suggestions (PER NOTE). Here is an example response:
...
Below is a list of notes from the course "{course}", delimited by '--note 1--', '--note 2--', etc. Each note is enclosed in quotations, so note 1 will look like this: \n\n--note 1--\n"[content]"\n\n You will use these to construct the JSON array of study suggestions, as detailed above.
